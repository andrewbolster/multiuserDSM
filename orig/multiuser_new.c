...
#include <cstdio>
#include <cfloat>
double max_lead=-DBL_MAX;
double max_lag=DBL_MAX;
double normalised_channel_gain(int line_id,int channel);
double get_max_gain(void);
double f(double n_delta_xtalk_power,int line_id,int user,int channel);
double relative_intraline_goodness(int line_id,int channel);
double relative_interline_goodness(int line1,int line2);
bool DESC(const xtalker_metric &left,const xtalker_metric &right)
{
	return left.lost_bits > right.lost_bits;
}
multiuser_new::multiuser_new()
{
	...
	w = new double[lines];
	wp = new double[lines];
	wp_offset = new double[lines];
	wp_exempt = new bool[lines];
	wb = new double[lines];
	wxt = new double[lines];
	w_min = new double[lines];
	p_lead = new double [lines];
...
void multiuser_new::init_cost_matrix()
{
        for (int tone=0;tone<DMTCHANNELS;tone++) {
                for(int user=0;user<lines;user++) {
...
void multiuser_new::update_wp()
{
	double p_ave=0.0;
	int num=lines;
	for (int user=0;user<lines;user++) {
		if (!wp_exempt[user]) {
			p_ave+=p_used[user]*100/_p_budget[user];
		}
		else
			num--;
	}
	...
	for (int user=0;user<lines;user++) {
		...
		if (b_target[user] == NOT_SET) {
			//double x=(p_used[user]-p_ave-p_lead[user])*100/p_ave; 	// percentage difference from p_ave
			double x=(p_used[user]*100/_p_budget[user])-p_ave; 	// percentage difference from p_ave
			//wp[user] = UNdB(2*(x));
			wp[user] = UNdB(2000/(100*(p_ave+0.00001))*(x));
		}
		/*
		else {
			double x=(p_used[user]*100/p_budget);
			double delta_b = ((double)b_total[user]*100/(double)b_target[user] - pow(x,3)/10000);
			wp[user] = UNdB(0.1*delta_b);
		}*/
		//wp[user] = UNdB(exp(30*x)-exp(-30*x));	
		//wp[user] = UNdB(0);	
		//wp[user] = UNdB(200*(1-exp(-150*x))/(1+exp(-150*x)));	
		//wp[user] = UNdB(0.1*(exp(200*x)-exp(-200*x)));
		//wp[user] = UNdB(0.1*(exp(2000*x)));
		//wp[user] = UNdB(1e-5*(exp(8000*x)));
		//wp[user] = UNdB(0.5*(exp(1500*x)));
		double y = p_used[user]-p_ave;
		if (y > max_lead) {
			max_lead=y;
		}
		if (y < max_lag) {
...
bool multiuser_new::all_rates_good()
{
	bool ret=true;
	for (int user=0;user<lines;user++) {
		if (b_target[user] == NOT_SET) {
...
int multiuser_new::update_power(int tone, int user)
{
...
int multiuser_new::calc_lost_bits()
{	
	struct line* current;
	double bits;
...
double multiuser_new::calc_delta_p(int channel,int line_id)
{
	...
	//cost[channel][line_id] = new_cf(channel,line_id,_p,_old_p,_b,g);
...
int multiuser_new::min_cost(int *channel,int *line)
{
	int tone,user;
...
double multiuser_new::xtalk_only_cost_function(int channel,int line_id,double*p,double *old_p,int *b)
{
...
double multiuser_new::cost_function(int channel,int line_id,double*p,double *old_p,int *b)
{
	double delta_p=0;
	double ret=0;
	delta_p=p[line_id]-old_p[line_id];
...
double multiuser_new::ya_cost_function(int channel,int line_id,double*p,double *old_p,int *b,double *g)
{
...
double multiuser_new::linear_cost_function(int channel,int line_id,double*p,double *old_p,int *b,double *g)
{
	double delta_p=p[line_id]-old_p[line_id];
	double pen=1;
	for (int user=0;user<lines;user++) {
...
double multiuser_new::cost_function1(int channel,int line_id,double*p,double *old_p,int *b,double *g)
{
...
double multiuser_new::last_cf(int channel,int line_id,double*p,double *old_p,int *b,double *g)
{
	...
	for (int user=0;user<lines;user++) {
		//delta_p+=wp[user]*(p[user]-old_p[user]);
...
//	getchar();
	...
	if (total_bits!=0) {
		...
		double diff;
		for (int user=0;user<lines;user++) {
			diff = (p_used[user] + _delta_p[channel][line_id][user]) - p_ave;
			if (diff > max_diff) {
				max_diff=diff;
			}	
		}
		//double p_pen = UNdB(5*max_diff);
		p_pen = UNdB(1e-5*exp(6e5*max_diff));
	}
	else {
		p_pen=1;
	}
	//return wp[line_id]*delta_p*pow(10,pen/10);
	//return delta_p*pow(10,pen/10);
	return wp[line_id]*delta_p;
	//return p_pen*delta_p*pow(10,pen/10);
}
double multiuser_new::new_cf(int channel,int user,double*p,double *old_p,int *b,double *g)
{
	double delta_p=0.0;
	for (int user=0;user<lines;user++) {
		//delta_p+=wp[user]*(p[user]-old_p[user]);
...
double multiuser_new::cf(int tone, int user)
{
	double delta_p=0.0;
	//printf("Calculating the cost of adding a bit to tone %d user %d\n",tone,user);
	for (int user1=0;user1<lines;user1++) {
		if (user==user1) {
			delta_p+=(wp[user1]*(1+wp_offset[user1]))*_delta_p[tone][user][user1];  	// wp[user] ??
			//printf("power penalty for line %d is %lf\n",user,wp[user]);
			//printf("Costs %10.8lf mW to add a bit\n",1e3*_delta_p[tone][user][user]);
		}
		else {
			delta_p+=wxt[user1]*(wp[user1]*(1-wp_offset[user1]/(lines-1)))*_delta_p[tone][user][user1];   // wxt[user1] to penalise xtalk into a line, wxt[user] to penalise xtalk
			//printf("power penalty for victim line %d is %lf\n",user1,wp[user1]);
			//printf("Costs %10.8lf mW on the victim to add a bit\n",1e3*_delta_p[tone][user][user1]);
		}
	}
	//printf("Total adjusted cost is %6.4lf\n",delta_p);
	return wb[user]*delta_p;
/*
	double ave=0.0;
	for (int user1=0;user1<lines;user1++) {
		p_cf[user1] = p_used[user1]+_delta_p[tone][user][user1];
		ave+=p_cf[user1];
	}
	ave/=lines;
	double var=0.0;
	for (int user1=0;user1<lines;user1++) {
		var+=pow((p_cf[user1]-ave),2);
	}
	var/=lines;
	return var;
*/
}
void multiuser_new::isb_optimise_p()
{
	double lk,lk_max;
	bool converged;
	int b_max;
	int *b_cur = new int [lines];
	...
	for (int tone=0;tone<DMTCHANNELS;tone++) {
		...
		while(1) {
			for (int user=0;user<lines;user++) {
				b_last[user]=b_cur[user];
			}
			for (int user=0;user<lines;user++) {
				for (b_cur[user]=0,lk=0.0,lk_max=-DBL_MAX;b_cur[user]<=MAXBITSPERTONE;b_cur[user]++) {
					lk = l_k(b_cur,tone);
					if (lk>lk_max) {
						lk_max=lk;
						b_max=b_cur[user];
						//printf("New lk_max on tone %d = %lf\n",tone,lk);
					}
				}
				b_cur[user]=b_max;
			}
		...
		printf("Value of lk found by isb on tone %d is %lf\n",tone,lk_max);
		double b_sum=0,p_sum=0.0;
		int *b_mn = new int [lines];
		print_vector(b_cur,"b_isb");
		for (int user=0;user<lines;user++) {
			b_sum+=b[tone][user]*1;
			p_sum+=p[tone][user]*l[user];
			b_mn[user]=b[tone][user];
		}
		printf("Value of lk in mn is %lf\n",b_sum-p_sum);
		print_vector(b_mn,"b_mn");		
		getchar();
	}
	delete[] b_cur;
	delete[] b_last;
}
double multiuser_new::l_k(int *b_cur,int tone) 
{
	...
	for (int user=0;user<lines;user++) {
		...
		p_sum+=l[user]*_p[user];	
	}
	delete[] _p;
	delete[] g;
	return b_sum-p_sum;
}
void multiuser_new::sort_lost_bits()
{
 	for (int user=0;user<lines;user++) {
...
void multiuser_new::print_lost_bits()
{
	for (int user=0;user<lines;user++) {
		for (int xtalker=0;xtalker<lines;xtalker++) {
...
double multiuser_new::f(double n_delta_xtalk_power,int line_id,int user,int channel)
{
...
double multiuser_new::relative_intraline_goodness(int line_id,int channel)
{
...
double multiuser_new::relative_interline_goodness(int line1,int line2)
{
	return ave_bits[line1]/ave_bits[line2];
}
double multiuser_new::relative_interline_goodness_channel(int line1,int line2,int channel)
{
	return bits[line1][channel]/bits[line2][channel];
}
void multiuser_new::calc_ave_bits()
{
	for (int user=0;user<lines;user++) {
		ave_bits[user]=0;
		ave_gain[user]=0;
...
void multiuser_new::calc_ave_xtalk()
{
	for (int user=0;user<lines;user++) {
		for (int xtalker=0;xtalker<lines;xtalker++) {
...
double multiuser_new::weighting(int line_id)
{
	double factor;
...
void multiuser_new::set_loading_algo()
{
	int user;
	struct line* current;
	for (user=0;user<lines;user++) {
		current=get_line(user);
		strcpy(current->loading_algo,"multiuser new");
	}
}
void multiuser_new::write_stats_totals()
{
	char fn[80];
	FILE *fp;
	sprintf(fn,"%s/data/mn_test/stats/stats.txt",ROOT_DIR);
	fp = fopen(fn,"w");
	if (fp==NULL)
		exit(2);
	fprintf(fp,"#%dlines\n",lines);
...
void multiuser_new::write_current_stats(int tone,int user)
{
	char fn[80];
	char fn1[80];
	FILE *fp;
	FILE *fp1;
	sprintf(fn,"%s/data/mn_test/stats/%d.txt",ROOT_DIR,total_bits);
	sprintf(fn1,"%s/data/mn_test/stats/b_and_p_stats.txt",ROOT_DIR);
	fp = fopen(fn,"w");
	if (fp==NULL)
		exit(2);
	fp1 = fopen(fn1,"a");
	...
	for (int tone=0;tone<DMTCHANNELS;tone++) {
		...
		*/
		if (tone < lines) {
			fprintf(fp,"%6.4lg\t",10*log10(p_used[tone]*1e3));
			fprintf(fp,"%6.4lg\t",wp[tone]);
			fprintf(fp,"%d\t",b_total[tone]);
		}
		if (tone == 0) {
			double p_ave=0.0;
			for (int user=0;user<lines;user++) {
			...
			p_ave/=lines;
			fprintf(fp,"%6.4g",p_ave*1e3);
		}
		fprintf(fp,"\n");
	}
	fprintf(fp1,"%d\t",total_bits);
	for (int user=0;user<lines;user++) {
		fprintf(fp1,"%d   ",b_total[user]);
	}
	for (int user=0;user<lines;user++) {
		fprintf(fp1,"%6.4lg   ",p_used[user]);
	}
	fprintf(fp1,"\n");
	fclose(fp);
	fclose(fp1);
}
void multiuser_new::write_mw_stats(int line_id,char *tag)
{
	int i;
	struct line* c;
	char fn[255];
...
bool multiuser_new::total_power_constraint_broken(int tone,int user)
{
	for (int i=0;i<lines;i++) {
