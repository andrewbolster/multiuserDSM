...
void optimise_p(struct isb_2_params *p)
{
	...
	for (i=0;i<DMTCHANNELS;i++) {
		...
		check_osb(&b1check,&b2check,i,p);
		if (b1max != b1check || b2max != b2check) {
			int c;
			printf("b1 = %d b2 = %d\n",b1max,b2max);
			printf("max not found, max b1 = %d max b2 = %d lkmax = %lf\n",b1check,b2check,l_k(b1check,b2check,i,p));
			printf("ok ? > ");
			c = getc(stdin);
			if ( c == 'n') {
				exit(3);
